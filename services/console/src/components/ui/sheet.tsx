...
const sheetVariants = cva(
    "fixed z-[100] gap-4 bg-background p-6 shadow-2xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
    {
        variants: {
            side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
        ...
        >
            {children}
            <DialogPrimitive.Close className="absolute right-4 top-4 z-10 flex size-7 items-center justify-center rounded-none border border-border bg-muted/50 text-muted-foreground opacity-100 ring-offset-background transition-all hover:bg-muted hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
                <X className="h-4 w-4" />
