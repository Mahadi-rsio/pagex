...
export function validateDeploymentManifest(
    ...
    | { valid: true; manifest: DeploymentManifest }
    ...
    if (m.version !== DEPLOYMENT_MANIFEST_VERSION) {
        return {
            valid: false,
            error: `Unsupported manifest version: ${String(m.version)}`,
        };
    }
    ...
    for (const [rawPath, rawHash] of Object.entries(
        ...
        if (!hash) {
            return {
                valid: false,
                error: `Invalid blob hash for path ${path}`,
            };
        }
