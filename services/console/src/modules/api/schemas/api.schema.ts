...
export const siteDailyStats = pgTable(
    "site_daily_stats",
    {
        ...
        requests2xx: bigint("requests_2xx", { mode: "number" })
            .notNull()
            .default(0),
        requests3xx: bigint("requests_3xx", { mode: "number" })
            .notNull()
            .default(0),
        requests4xx: bigint("requests_4xx", { mode: "number" })
            .notNull()
            .default(0),
        requests5xx: bigint("requests_5xx", { mode: "number" })
            .notNull()
            .default(0),
        humans: bigint("humans", { mode: "number" }).notNull().default(0),
        bots: bigint("bots", { mode: "number" }).notNull().default(0),
        uniqueIps: bigint("unique_ips", { mode: "number" })
            .notNull()
            .default(0),
        peakHour: text("peak_hour"),
        peakHourRequests: bigint("peak_hour_requests", { mode: "number" })
            .notNull()
            .default(0),
        updatedAt: timestamp("updated_at").defaultNow(),
    },
    (t) => ({
        uniqueSiteDate: unique("site_daily_stats_site_id_date_uid").on(
            t.siteId,
            t.date,
        ),
        siteDateIdx: index("idx_site_daily_stats_site_date").on(
            t.siteId,
            t.date,
        ),
    }),
 ...
 */
export const bandwidthUsageHourly = pgTable(
    ...
    (t) => ({
        pk: primaryKey({
            name: "bandwidth_usage_hourly_pk",
            columns: [t.tenantId, t.siteId, t.bucket],
        }),
        tenantBucketIdx: index("idx_bandwidth_usage_tenant_bucket").on(
            t.tenantId,
            t.bucket,
        ),
        siteBucketIdx: index("idx_bandwidth_usage_site_bucket").on(
            t.siteId,
            t.bucket,
        ),
    }),
 ...
 */
export const serviceMetricsHourly = pgTable(
    "service_metrics_hourly",
    {
        ...
        bucket: timestamp("bucket", { withTimezone: true }).notNull(),
        requests: bigint("requests", { mode: "number" }).notNull().default(0),
        status2xx: bigint("status_2xx", { mode: "number" })
            .notNull()
            .default(0),
        status3xx: bigint("status_3xx", { mode: "number" })
            .notNull()
            .default(0),
        status4xx: bigint("status_4xx", { mode: "number" })
            .notNull()
            .default(0),
        status5xx: bigint("status_5xx", { mode: "number" })
            .notNull()
            .default(0),
        bytes: bigint("bytes", { mode: "number" }).notNull().default(0),
        cacheHits: bigint("cache_hits", { mode: "number" })
            .notNull()
            .default(0),
        cacheMisses: bigint("cache_misses", { mode: "number" })
            .notNull()
            .default(0),
        latencySumMs: bigint("latency_sum_ms", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe50: bigint("latency_le_50", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe100: bigint("latency_le_100", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe250: bigint("latency_le_250", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe500: bigint("latency_le_500", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe1000: bigint("latency_le_1000", { mode: "number" })
            .notNull()
            .default(0),
        latencyLe2500: bigint("latency_le_2500", { mode: "number" })
            .notNull()
            .default(0),
        updatedAt: timestamp("updated_at", { withTimezone: true })
            .defaultNow()
    ...
    (t) => ({
        pk: primaryKey({
            name: "service_metrics_hourly_pk",
            columns: [t.siteId, t.bucket],
        }),
        siteBucketIdx: index("idx_service_metrics_site_bucket").on(
            t.siteId,
            t.bucket,
        ),
...
export const idempotencyKeys = pgTable(
    ...
    (t) => ({
        uniqueTenantPageKey: unique("idempotency_keys_tenant_page_key_uid").on(
            t.tenant_id,
            t.page_id,
            t.idempotency_key,
        ),
        expiresIdx: index("idx_idempotency_keys_expires").on(t.expires_at),
        tenantPageIdx: index("idx_idempotency_keys_tenant_page").on(
            t.tenant_id,
            t.page_id,
        ),
        resourceIdx: index("idx_idempotency_keys_resource").on(
            t.resource_type,
            t.resource_id,
        ),
 ...
 */
export const buildFailures = pgTable(
    "build_failures",
    {
