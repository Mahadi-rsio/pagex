...
async function loadDeployToken(
    token: string,
    tenantId: string,
...
async function storeExpandedVariants(
    ...
    const expanded = await Promise.all(
        sources.map((source) =>
            limit(async () => {
                const result = await expandFileVariants(
                    source.path,
                    source.body,
                    log,
                );
...
export async function commitBlobTreeDeploy(opts: {
    ...
    const run = async () => {
        ...
        if (!newDep)
            throw new HttpError("Failed to create deployment record", 500);
        ...
        const [newer] = await db
            ...
            .where(
                and(
                    eq(deployments.page_id, opts.pageId),
                    gt(deployments.version, newDep.version),
                ),
            )
            .limit(1);
        ...
        async () => {
            ...
            try {
                ...
                const [failedDep] = await db
                    ...
                    .where(
                        and(
                            eq(deployments.build_id, opts.buildId),
...
export async function prepareDeploy(
    ...
    for (const file of input.files) {
        if (seenPaths.has(file.path)) {
            throw new HttpError(
                `Duplicate path in manifest: ${file.path}`,
                400,
            );
        }
    ...
    const idempotencyKey =
        input.idempotencyKey ?? randomBytes(24).toString("hex");
        ...
        if (existingToken) {
            return {
                ...
                summary: {
                    ...
                    totalSize,
                    totalSizeHuman: formatBytes(totalSize),
                    uploadSize: uploadRequired.reduce(
                        (sum, f) => sum + f.size,
                        0,
                    ),
...
export async function presignDeploy(
    ...
    for (const hash of input.hashes) {
        if (!manifestHashes.has(hash)) {
            throw new HttpError(
                `Hash ${hash} is not in the deployment manifest`,
                400,
            );
        }
