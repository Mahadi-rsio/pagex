...
export async function checkAndReserveIdempotencyKey(opts: {
    ...
    const requestHash = createHash("sha256")
        .update(JSON.stringify(opts.requestBody))
        .digest("hex");
...
export async function findCompletedIdempotencyByKey(opts: {
    ...
    if (!result || result.status !== "completed" || !result.resource_id)
        return null;
...
export async function cleanupExpiredIdempotencyKeys(): Promise<number> {
    ...
    await db.delete(idempotencyKeys).where(lt(idempotencyKeys.expires_at, now));
