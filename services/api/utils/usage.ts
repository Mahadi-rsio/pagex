import { BYTES_PER_GB, getPlan, type PlanId } from '../constants/pricing.js'
...
export function computeBandwidthUsage(
    ...
    const used = Math.max(0, Math.trunc(Number.isFinite(usedBytes) ? usedBytes : 0))
    ...
    const percentage = limit === 0 ? 0 : Number(((used / limit) * 100).toFixed(2))
...
export function isOverQuota(plan: PlanId | string | null | undefined, usedBytes: number): boolean {
