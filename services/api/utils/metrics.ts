...
function cumulativePairs(input: LatencyInput): Array<{ bound: number; count: number }> {
    return [
 ...
 */
export function estimatePercentile(input: LatencyInput, requests: number, percentile: number): number | null {
...
export function summarizeLatency(input: LatencyInput, requests: number): LatencySummary {
    const samples = input.latencyLe2500
    return {
        averageMs: requests > 0 ? Number((input.latencySumMs / requests).toFixed(2)) : null,
...
export function buildMetricsResponse(input: MetricsInput): MetricsResponse {
    ...
    return {
        ...
        cache: {
            ...
            hitRate: cacheTotal > 0 ? Number(((input.cacheHits / cacheTotal) * 100).toFixed(2)) : null,
        },
...
export function resolveMetricsWindow(
    ...
    if (query.from) {
        ...
        start = Number.isNaN(parsed.getTime()) ? new Date(validEnd.getTime() - 24 * 3600_000) : parsed
    } else {
        const named = NAMED_WINDOW_HOURS[(query.window ?? '').toLowerCase()] ?? 24
