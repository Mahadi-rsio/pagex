...
import { BYTES_PER_GB, getPlan, PLANS } from '../constants/pricing.js'
import {
    buildUsageResponse,
    bytesToGB,
...
test('computeBandwidthUsage never returns a request field', () => {
    const usage = computeBandwidthUsage('free', 1) as unknown as Record<string, unknown>
...
test('buildUsageResponse exposes bandwidth only and ISO period bounds', () => {
    const res = buildUsageResponse('free', 25 * BYTES_PER_GB, currentUsageWindow(new Date('2026-09-23T00:00:00Z')))
