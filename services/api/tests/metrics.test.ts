...
test('estimatePercentile returns the smallest covering bound', () => {
    ...
    const input = latency({ latencyLe50: 100, latencyLe100: 100, latencyLe250: 100, latencyLe500: 100, latencyLe1000: 100, latencyLe2500: 100 })
...
test('estimatePercentile walks cumulative buckets', () => {
    ...
    const input = latency({ latencyLe50: 90, latencyLe100: 90, latencyLe250: 100, latencyLe500: 100, latencyLe1000: 100, latencyLe2500: 100 })
    assert.equal(estimatePercentile(input, 100, 50), 50)
...
test('summarizeLatency computes the average from the sum', () => {
    const summary = summarizeLatency(latency({ latencySumMs: 1000, latencyLe50: 10, latencyLe100: 10, latencyLe250: 10, latencyLe500: 10, latencyLe1000: 10, latencyLe2500: 10 }), 10)
...
test('resolveMetricsWindow honours named windows', () => {
    const now = new Date('2026-09-23T12:00:00Z')
    assert.equal(resolveMetricsWindow({ window: '1h' }, now).start.toISOString(), '2026-09-23T11:00:00.000Z')
    assert.equal(resolveMetricsWindow({ window: '7d' }, now).start.toISOString(), '2026-09-16T12:00:00.000Z')
})

test('resolveMetricsWindow clamps the span to 31 days', () => {
    const now = new Date('2026-09-23T12:00:00Z')
    const { start } = resolveMetricsWindow({ from: '2026-01-01T00:00:00Z', to: now.toISOString() }, now)
